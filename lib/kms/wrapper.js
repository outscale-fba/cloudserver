...
if (config.backends.kms === 'mem') {
    ...
    client = new AWSClient(awsConfig, logger);
    implName = 'aws';
} else {
...
class KMS {
    ...
    static createBucketKey(bucket, log, cb) {
        ...
        client.createBucketKey(bucket, log, (err, masterKeyId) => {
            if (err) {
    ...
    static bucketLevelEncryption(bucket, sseConfig, log, cb) {
        ...
        */
        ...
        if (algorithm === 'AES256' || algorithm === 'aws:kms') {
            return this.createBucketKey(bucket, log, (err, masterKeyId) => {
                if (err) {
                    return cb(err);
                }
            ...
            function generateDataKey(next) {
                ...
                */
                if (client.generateDataKey) {
                    ...
                    return client.generateDataKey(cipherBundle.cryptoScheme,
