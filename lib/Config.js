...
class Config extends EventEmitter {
    ...
    _getConfig() {
        ...
        if (config.kmsAWS) {
            const { region, endpoint, ak, sk, perUserKey } = config.kmsAWS;
            if (region) {
            ...
            */
            ...
            /* Use a per-user key instead of per-bucket key.
               The key is handled (stored in the buckets / objects metadata) by cloudserver
               in the exact same way of a per bucket key.
               The KMS connector is reponsible for making the distinction.
            */
            if (perUserKey) {
                this.kmsAWS.perUserKey = perUserKey;
            }
        }
