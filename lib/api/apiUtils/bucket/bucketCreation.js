...
function createBucket(authInfo, bucketName, headers,
    ...
    (err, results) => {
        ...
        if (existingBucketMD === 'NoBucketYet') {
            ...
            // No encryption in header and default values configured
            if (!sseConfig.mandatory
                    && config.defaultBucketSseConfig.algorithm
                    && config.defaultBucketSseConfig.mandatory) {
                sseConfig.algorithm = config.defaultBucketSseConfig.algorithm;
                sseConfig.mandatory = config.defaultBucketSseConfig.mandatory;
            }
